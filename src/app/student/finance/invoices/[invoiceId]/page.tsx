...
export default async function StudentInvoiceDetailPage({ params }: Props) {
  const { supabase, schoolId, student } = await requireStudentCtx()

  const { data: rawInv } = await supabase
    .from('student_invoices')
    .select('id, invoice_number, title, total_amount, amount_paid, status, due_date, created_at, student_id')
  ...
  // Ownership: the invoice must belong to this student.
  if (inv.student_id !== student.id) notFound()
  ...
  return (
    <div className="space-y-6 pb-8">
      <div className="rounded-xl bg-primary-800 px-6 py-5">
        <div className="mb-1">
          <a href="/student/finance" className="text-primary-300 hover:text-white text-sm">← Finance</a>
        </div>
        ...
        <p className="mt-0.5 text-sm text-primary-300">Échéance {fmtDate(inv.due_date)}</p>
      </div>
