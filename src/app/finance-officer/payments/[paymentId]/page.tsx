...
export default async function FinanceOfficerReceiptPage({ params }: Props) {
  const { supabase, schoolId } = await requireFinanceOfficerCtx()

  const { data: rawPayment } = await supabase
    .from('student_payments')
    .select('id, receipt_number, amount, payment_method, reference, notes, paid_at, created_at, schools!school_id(name, phone, email, address), students!student_id(first_name, last_name), student_invoices!invoice_id(id, invoice_number, title)')
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between print-hide">
        {invoice ? (
          <a href={`/finance-officer/invoices/${invoice.id}`} className="text-sm text-primary-600 hover:text-primary-800 hover:underline">← Retour à la facture</a>
        ) : (
          <a href="/finance-officer/payments" className="text-sm text-primary-600 hover:text-primary-800 hover:underline">← Journal des paiements</a>
        )}
        <PrintButton />
      </div>
      ...
      <div id="receipt" className="mt-4 mx-auto max-w-lg rounded-xl border border-sand-200 bg-white shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-5 divide-y divide-sand-100">
        ...
        <div className="border-t border-sand-200 px-6 py-4 text-center">
          <p className="text-xs text-gray-400">Enregistré le {new Date(payment.created_at).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
          <p className="text-xs text-gray-300 mt-0.5">Ce reçu est émis par {school.name}</p>
        </div>
      </div>
    </>
  )
