...
import { recordFinanceOfficerPayment, type FoPaymentState } from '../actions'
...
const initialState: FoPaymentState = {}

export function FinanceOfficerPaymentForm({ invoiceId, balance }: { invoiceId: string; balance: number }) {
  const [state, formAction] = useFormState(recordFinanceOfficerPayment, initialState)
    ...
    <form action={formAction} noValidate className="space-y-4">
      ...
      <div>
        ...
        <div className="relative">
          <input
            id="pay_amount" name="amount" type="number" min="1" step="1"
            defaultValue={balance > 0 ? balance : ''} placeholder="0"
        ...
        {balance > 0 && <p className="mt-1 text-xs text-gray-500">Solde restant : {fmt(balance)}</p>}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <select
            id="payment_method" name="payment_method" defaultValue="cash"
          ...
          {state.errors?.payment_method && <p className="mt-1 text-xs text-red-600">{state.errors.payment_method[0]}</p>}
        </div>
        <div>
          <label htmlFor="paid_at" className="block text-sm font-medium text-gray-700 mb-1">
            Date de paiement <span className="font-normal text-gray-400">(optionnel)</span>
          </label>
          <input
          ...
          />
          {state.errors?.paid_at && <p className="mt-1 text-xs text-red-600">{state.errors.paid_at[0]}</p>}
        </div>
      </div>
      ...
      <div>
        ...
        <input
          id="reference" name="reference" type="text" placeholder="N° reçu, N° transaction…"
        ...
        />
        {state.errors?.reference && <p className="mt-1 text-xs text-red-600">{state.errors.reference[0]}</p>}
      </div>
      ...
      <div>
        ...
        <textarea
          id="pay_notes" name="notes" rows={2} placeholder="Observations…"
          className="block w-full rounded-md border border-gray-300 px-3 py-2 text-sm text-gray-900 placeholder-gray-400 shadow-sm focus:border-primary-600 focus:outline-none focus:ring-1 focus:ring-primary-600"
        />
      </div>
