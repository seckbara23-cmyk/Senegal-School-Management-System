...
import { createAcademicYear, type AcademicYearFormState } from '../actions'
...
function SubmitButton() {
  ...
  return (
    ...
    >
      {pending ? "Enregistrement…" : "Créer l'année scolaire"}
    </button>
  )
}
...
export function AcademicYearCreateForm() {
  const [state, formAction] = useFormState(createAcademicYear, initialState)
    ...
    <form action={formAction} noValidate className="space-y-5">
      ...
      <div className="rounded-lg border border-sand-200 bg-sand-50 px-4 py-3 space-y-3">
        <label className="flex cursor-pointer items-start gap-3">
          ...
          />
          <div>
            <p className="text-sm font-medium text-gray-900">Marquer comme année en cours</p>
