...
export default async function StudentDetailPage({ params, searchParams }: Props) {
  ...
  // Student 360 narrative — shared deterministic engine, reusing the snapshot
  // already loaded above (no extra query, no DB access in the engine).
  const narrative = generateStudentNarrative(
    studentRisk
      ? { ...studentRisk, outstanding }
      : { studentId: s.id, firstName: s.first_name, lastName: s.last_name, classId: '', className: '—', average: null, level: 'low', score: 0, reasons: [], actions: [], outstanding },
  )
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── Synthèse 360 (narrative dérivée, jamais stockée) ─────────────────── */}
