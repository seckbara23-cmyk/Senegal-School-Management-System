...
export default async function AssessmentDetailPage({ params, searchParams }: Props) {
  ...
  // Fetch enrolled students + existing grades in parallel
      ...
      {enrollments.length === 0 ? (
        <div className="rounded-xl border-2 border-dashed border-sand-300 bg-sand-50 py-12 px-6 text-center">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Inscrivez des élèves actifs dans cette classe pour saisir des notes.
          </p>
          <a
            href={`/school/classes/${assessment.class_subjects.class_id}`}
          ...
          <form action={saveGrades}>
            ...
            <div className="flex items-center justify-between gap-4 border-t border-sand-200 bg-sand-50 px-5 py-3">
              <p className="text-xs text-gray-400">
                Laissez vide pour effacer une note existante.
              </p>
              <button
                type="submit"
                    ...
                    >
                      ...
                      <td className="px-4 py-2.5 text-right">
                        <span className={`font-semibold ${
                          g.score / maxScore >= 0.5 ? 'text-emerald-700' : 'text-red-600'
                        }`}>
