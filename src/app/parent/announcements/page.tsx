...
export default async function ParentAnnouncementsPage() {
  ...
  // Get the classes of each linked child (for class-specific announcements)
  const { data: linksData } = await supabase
    .from('parent_student_links')
    .select('student_id')
    .eq('parent_id', parent.id)

  const childIds = (linksData ?? []).map((l) => l.student_id)
  ...
  if (childIds.length > 0) {
    const { data: enrData } = await supabase
      ...
      .eq('status', 'active')
    classIds = (enrData ?? []).map((e) => (e as { class_id: string }).class_id)
  }

  // Fetch announcements: all_school + parents + class (for children's classes)
  const { data: rawAnn } = await supabase
    .from('announcements')
    .select('id, title, body, audience_type, class_id, created_at, classes!class_id(name)')
    .eq('school_id', schoolId)
    .in('audience_type', ['all_school', 'parents', 'class'])
  ...
  // Filter class announcements to only children's classes
