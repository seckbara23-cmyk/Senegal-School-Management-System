...
import { startThreadParent, type MsgState } from '../actions'
...
export function NewMessageForm({ options }: { options: { value: string; label: string }[] }) {
  const [state, formAction] = useFormState(startThreadParent, {} as MsgState)
  return (
    <form action={formAction} className="space-y-4 rounded-xl border border-sand-200 bg-white p-5 shadow-sm">
      ...
      <div>
        ...
        <input id="subject" name="subject" type="text" maxLength={150} placeholder="Ex. Suivi en mathématiques" className={field} />
      </div>
      ...
      <div className="flex items-center gap-3 pt-1">
        <SubmitButton />
        <a href="/parent/messages" className="text-sm text-gray-600 hover:underline">Annuler</a>
      </div>
    </form>
  )
}
