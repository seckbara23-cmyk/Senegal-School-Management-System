...
export default async function TeacherAttendanceSessionPage({ params }: Props) {
  ...
  const { data: raw } = await supabase
    ...
    .select(
      'id, session_date, notes, class_id, ' +
  ...
  // Verify session belongs to a class the teacher is assigned to
  ...
  if (assignedClassSubjectIds.length > 0) {
    ...
    isAssigned = ((csRows ?? []) as { id: string }[]).length > 0
  }
    ...
    <div className="space-y-5 pb-8">
      ...
      <div className="rounded-xl bg-primary-800 px-6 py-5">
        ...
        <p className="text-primary-300 text-sm mt-0.5">
          {className} · {session.academic_years.name}
        </p>
      </div>
      ...
      <div className="overflow-hidden rounded-xl grid grid-cols-2 sm:grid-cols-4 shadow-sm">
        <div className="bg-primary-600 px-5 py-5 text-center">
          ...
          <p className="text-xs font-semibold uppercase tracking-wider text-primary-200 mt-1">Présents</p>
          <p className="text-sm font-medium text-primary-100 mt-1">{pct(counts.present)} %</p>
        </div>
        <div className="bg-red-600 px-5 py-5 text-center">
          <p className="text-3xl font-bold text-white">{counts.absent}</p>
          <p className="text-xs font-semibold uppercase tracking-wider text-red-200 mt-1">Absents</p>
          <p className="text-sm font-medium text-red-100 mt-1">{pct(counts.absent)} %</p>
        </div>
        <div className="bg-amber-500 px-5 py-5 text-center">
          <p className="text-3xl font-bold text-white">{counts.late}</p>
          <p className="text-xs font-semibold uppercase tracking-wider text-amber-100 mt-1">En retard</p>
          <p className="text-sm font-medium text-amber-50 mt-1">{pct(counts.late)} %</p>
        </div>
        <div className="bg-sky-600 px-5 py-5 text-center">
          <p className="text-3xl font-bold text-white">{counts.excused}</p>
          <p className="text-xs font-semibold uppercase tracking-wider text-sky-200 mt-1">Excusés</p>
          <p className="text-sm font-medium text-sky-100 mt-1">{pct(counts.excused)} %</p>
        </div>
      </div>
          ...
          <table className="min-w-full">
            <thead>
              <tr className="border-b border-sand-200 bg-sand-50">
                <th scope="col" className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Nom</th>
                <th scope="col" className="hidden px-5 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 sm:table-cell">{`N° Adm.`}</th>
                <th scope="col" className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Statut</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-sand-100">
              {sortedRecords.map((r) => (
                <tr key={r.id} className="odd:bg-white even:bg-sand-50">
                  ...
                  <td className="px-5 py-3.5">
                    {r.status === 'present' && <span className="text-sm font-semibold text-primary-600">● Présent</span>}
                    {r.status === 'absent'  && <span className="text-sm font-semibold text-red-600">● Absent</span>}
                    {r.status === 'late'    && <span className="text-sm font-semibold text-amber-600">● En retard</span>}
                    {r.status === 'excused' && <span className="text-sm font-semibold text-sky-600">● Excusé</span>}
                  </td>
                </tr>
              ))}
            </tbody>
